...
import { Project, ProjectFormData, ProjectPriority, ProjectStatus, Entity, RecurrentExpense } from '../types';
...
export function ProjectModal({ isOpen, onClose, onSave, project }: ProjectModalProps) {
    ...
    // Expenses State
    const [expenses, setExpenses] = useState<Omit<RecurrentExpense, 'id' | 'project_id' | 'created_at'>[]>([]);
    const [newExpense, setNewExpense] = useState({ description: '', amount: 0, frequency: 'mensual' as const });
    ...
    useEffect(() => {
        if (project) {
            setFormData({
                ...
                expenses: [] // Handled separately via state
            });
            // Load existing expenses if available
            if (project.expenses) {
                setExpenses(project.expenses.map(e => ({
    ...
    // ... (fetchEntities useEffect remains same) ...
    useEffect(() => {
        const fetchEntities = async () => {
            ...
            if (profile?.has_all_entities_access) {
                ...
                if (profileEntities) {
                    const mappedEntities = profileEntities.map((pe: any) => pe.entity).filter(Boolean);
    ...
    const handleAddExpense = () => {
        ...
        setNewExpense({ description: '', amount: 0, frequency: 'mensual' });
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setIsSaving(true);
            // Combine form data with expenses
            const submissionData = {
                ...formData,
                expenses: formData.has_budget ? expenses : []
            };
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="glass-card w-full max-w-2xl max-h-[90vh] overflow-y-auto relative border border-white/20 shadow-2xl animate-in zoom-in-95 duration-300">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 space-y-4">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <h4 className="font-black text-foreground">Gestión Presupuestal</h4>
                            </div>
                            <label className="relative inline-flex items-center cursor-pointer">
                                ...
                                <span className="ml-3 text-sm font-medium text-slate-900 dark:text-slate-300">
                                    {formData.has_budget ? 'Habilitado' : 'Deshabilitado'}
                                </span>
                            </label>
                        </div>
                        ...
                        {formData.has_budget && (
                            <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                                <div className="space-y-1.5">
                                    <label className="text-sm font-bold text-foreground">Presupuesto General (CAPEX)</label>
                                    <div className="relative group">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-8 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-bold text-lg"
                                            placeholder="0.00"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                                    <label className="text-xs font-black uppercase text-muted-foreground tracking-widest mb-3 block">Gastos Recurrentes (OPEX)</label>

                                    <div className="flex gap-2 mb-3">
                                        <input
                                            type="text"
                                            placeholder="Descripción (ej: Servidor)"
                                        ...
                                        <input
                                            type="number"
                                            placeholder="Monto"
                                            className="w-24 px-3 py-2 rounded-lg border border-slate-200 text-sm font-medium"
                                            value={newExpense.amount || ''}
                                            onChange={(e) => setNewExpense({ ...newExpense, amount: Number(e.target.value) })}
                                        />
                                        <select
                                            className="w-24 px-2 py-2 rounded-lg border border-slate-200 text-sm font-medium"
                                    ...
                                    <div className="space-y-2 max-h-[150px] overflow-y-auto">
                                        {expenses.length === 0 && <p className="text-xs text-center text-muted-foreground py-2 italic">Sin gastos recurrentes registrados</p>}
                                        {expenses.map((exp, idx) => (
                                            <div key={idx} className="flex items-center justify-between p-2 bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-1 h-8 bg-purple-500 rounded-full" />
                                                    <div>
                                                        <p className="text-xs font-bold">{exp.description}</p>
                                                        <p className="text-[10px] text-muted-foreground uppercase">{exp.frequency}</p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    <span className="text-sm font-black">${exp.amount.toLocaleString()}</span>
                                                    <button type="button" onClick={() => handleRemoveExpense(idx)} className="text-red-400 hover:text-red-500">
                                                        <X className="w-3 h-3" />
                                                    </button>
                                                </div>
                                            </div>
                    ...
                    {/* Should remove contract active/support from main view to simplify? Keeping for now but grouped */}
